...
df = pandas.DataFrame.from_dict(dict(dataset))
df.index = description[1]
...
def get_year_data(data, year):
    try:
        partial_dataframe = pandas.DataFrame(data.loc[year])
        return {year: list(partial_dataframe.to_dict()[year].items())}
    except Exception:
        return "Wrong input"
...
def get_country_data(data, country):
    try:
        partial_dataframe = pandas.DataFrame(data.loc[:, country])
        return {country: list(partial_dataframe.to_dict()[country].items())}
    except Exception:
        return "Wrong input"
...
# print(get_year_data(df, "2019 "))  # DONE
# print(get_country_data(df, "RO"))  # DONE
...
# df.columns = ["coverage"]
# print(df["RO"].to_json(orient="split"))
